...
def calculate_predefined(request, election: str):
    ...
    eff_support = "eff-support" in request.GET
    
    try:
        result_election = run_pabutools_analytics(file_path, exhaust, eff_support)
...
def calculate_uploaded(request):
    ...
    try:
        result_election = run_pabutools_analytics("project_loss/temp.pb", exhaust, eff_support)
    except Exception as e:
        return HttpResponse(json.dumps(str(e)), status=422)
